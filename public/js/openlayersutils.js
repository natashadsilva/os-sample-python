 ...
 //period : how often to refresh
 //updateFn: function to call after request
 function periodicUpdateJSON(url, params, period, updateFn) {
    getData = function(url, params, updateFn) {
      makeRequest(url, params, updateFn)
   };

    getData(url, params, updateFn);
    return setInterval(function() {getData(url, params, updateFn);}, period* 1000);
};

function makeRequest3(url, params, updateFn){

   var xmlhttp = new XMLHttpRequest();
   xmlhttp.onreadystatechange = function() {
      if (this.readyState == 4 && this.status == 200) {
         updateFn(JSON.parse(this.responseText));
      }
   };
...
function makeRequest(url, params, updateFn){
   $.post({
      data: params,
      url: url,
      dataType: "json",
...
getMarkerGraphic = function( markerType) {
    ...
    switch (markerType) {
     ...
     case 'BLUE':
          return 'img/marker-blue.png';
     case 'WARNING':
          return 'img/marker-warning.png';
     case 'AWARD':
          return 'img/marker-award.png';
     default:
         return markerType;
    }
}
